...
document.addEventListener('DOMContentLoaded', () => {
    // Overlay panel toggle for sign in/sign up
    ...
    if (registerForm && container) {
        registerForm.addEventListener('submit', function(e) {
            ...
            createUserWithEmailAndPassword(auth, email, password)
                .then(() => {
                    showFormMessage(registerForm, 'Registration successful! Please sign in with your new credentials.', false);
                    setTimeout(() => {
                        container.classList.remove('right-panel-active');
                    }, 1200);
                })
    ...
    document.querySelectorAll('.social[aria-label*="Google"]').forEach(btn => {
        btn.addEventListener('click', async (e) => {
            ...
            try {
                ...
                // Try to show error on the visible form, fallback to alert
